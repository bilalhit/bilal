...
app.post('/api/iteams',(req ,res)=>{
    ...
    let Query = `INSERT INTO iteams ?`;
    console.log(data);
    const query = con.query(Query, data,(err, results) => {
        if(err) {
            throw err;
        }
        else{
...
app.put('/api/iteams/:id',(req, res) => {
    let sqlQuery = "UPDATE iteams SET tital='"+req.body.title+"', body='"+req.body.body+"' WHERE id="+req.params.id;
